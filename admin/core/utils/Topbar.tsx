...
export default function Topbar() {
    ...
    return (
        <header className={`fixed w-full text-gray-800 bg-white shadow-md px-10 py-4 z-10 flex items-center justify-between`}>
            ...
            >
                {LINKS.map(
                    (link, index) =>
                        link.isShown && (
                            <Link href={link.path} key={index} className={`transition-colors duration-300 flex items-center gap-1 ${pathname === link.path && 'text-blue-800'}`}>
                                {link.name}
                            </Link>
                        )
                )}
